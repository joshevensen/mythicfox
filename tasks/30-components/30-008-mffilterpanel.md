...
- [x] `resources/js/components/MfFilterPanel.vue` exists with prop:
...
- [x] Per filter kind, the panel renders:
...
- [x] Active-filter chips render above the controls. Each chip shows `{label}: {value}` and an X icon to remove that one filter. Implemented via `<MfFilterChip>` in `resources/js/components/MfFilterChip.vue`.
- [x] "Clear all filters" button appears next to the chips when ≥1 filter is active.
- [x] All filter changes update the URL via Inertia `router.get(url, { preserveState: true })`. Multi-value enum filters serialize as comma-separated values (e.g. `?product=Magic,Lorcana`).
- [x] Panel is collapsible (PrimeVue `Panel` with `:collapsed` and a toggle). Default state: expanded on desktop, collapsed on tablet, full-screen drawer on mobile (per `docs/ux/ux-patterns.md#responsive-behavior`).
- [x] On mobile (`< 768px`), the panel renders as a PrimeVue `Drawer` triggered by a "Filters" button rendered by the consuming page (the panel exposes a `v-model:open` API for the drawer state so the page can wire its own trigger button).
- [x] `resources/js/components/MfSearchInput.vue` exists. Wraps PrimeVue `IconField` + `InputText` (or just `InputText`) with a 300ms debounce on the `update:modelValue` emit. Used by the panel for `text`-kind filters and standalone where a page wants a single search box.
