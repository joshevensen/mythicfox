...
- [ ] `resources/js/components/MfFormField.vue` exists with props:
  - `label: string`
...
- [ ] Default slot renders the actual input. Wrapper outputs:
...
- [ ] `resources/js/components/MfMoneyInput.vue` exists. Wraps PrimeVue `InputNumber` with:
...
- [ ] `resources/js/components/MfQtyInput.vue` exists. Wraps PrimeVue `InputNumber` with:
...
- [ ] `resources/js/components/MfDatePicker.vue` exists. Wraps PrimeVue `DatePicker` (formerly `Calendar` in v3, `DatePicker` in v4):
...
- [ ] All four components support dark mode (no hardcoded colors; PrimeVue tokens + Tailwind utilities only).
