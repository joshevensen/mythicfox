...
- [x] A `PublicFooter.vue` component exists and is rendered by `PublicLayout` (replacing the placeholder from `40-001`). It outputs a single muted line: `© 2025 – {currentYear} Mythic Fox Games  ·  [Admin]`.
...
- [x] The "Admin" link points to the `/login` route via Wayfinder. Discreet styling (muted text color, no button chrome).
- [x] No social links, no nav, no newsletter signup — exactly the spec from `public-homepage.md §Footer`.
- [x] Route `GET /sitemap.xml` returns a valid XML sitemap listing only `/` (admin routes excluded). Response `Content-Type: application/xml`.
- [x] Route `GET /robots.txt` returns `Allow: /` and `Disallow:` entries for `/login`, `/dashboard`, `/orders`, `/catalog`, `/inventory`, `/add-cards`, `/settings`, plus a `Sitemap:` line pointing to the absolute URL of `/sitemap.xml`. Response `Content-Type: text/plain`.
