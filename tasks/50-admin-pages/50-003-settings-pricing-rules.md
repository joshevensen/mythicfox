...
- [x] Route `GET /settings` registered under auth middleware, renders an Inertia page using `AdminLayout` from `50-001`. Page title `"Settings"`, subtitle `"Manage pricing rules and review import/export history."`
- [x] An inline TOC at the top of the page renders pill-link anchors for `#pricing-rules`, `#file-history`, and `#seller-stats`. (File History and Seller Stats are owned by `50-004` and `50-005`; render the anchors so all three pills exist on day one.)
- [x] The Pricing Rules section (`#pricing-rules`) renders one sub-section per product. Each product sub-section shows:
...
- [x] Clicking a product header / Edit icon opens the product rules modal:
...
- [x] Clicking a set row opens the set rules modal:
...
- [x] Server-side validation rejects `base_price > high_price` for both products and sets (effective values after inheritance is applied for sets).
- [x] First-time-visit empty state when no products exist: `"No products yet — they'll appear after your first PricingCustomExport upload."` with a link to `/catalog`.
- [x] Mobile (`< 768px`): the Pricing Rules sub-sections stack vertically, the entire product header row is tappable (not just the icon), and the modals become full-screen sheets. The "Inherit from {Product}" muted text sits on its own line beneath each input rather than beside it.
- [x] Pest feature test `tests/Feature/Admin/Settings/PricingRulesTest.php` covers:
