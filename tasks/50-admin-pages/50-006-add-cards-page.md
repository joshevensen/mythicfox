...
- [ ] Route `GET /add-cards` registered under auth middleware, renders an Inertia page using `AdminLayout` from `50-001`. Page header: title `"Add Cards"`, no primary header action.
- [ ] Top of page (scrolls away — **not** sticky): three single-select scope selectors:
...
- [ ] Card list renders only after all three selectors are set. Until then, render placeholder: `"Pick a product, set, and condition to add cards."`
- [ ] When scope is set with zero matching cards: `"No cards in {Set} match {Condition}. Try a different condition."`
- [ ] When scope yields cards, render an alphabetical scrollable list (no pagination — full result loaded at once). Each row:
  - **Left**: card name on top line; `#{Number}` muted small text below.
  - **Right**: `MfQtyInput` (− / integer / +). Default 0.
- [ ] Rows with qty > 0 render with a subtle green-tinted background and a checkmark glyph next to the qty.
- [ ] **Fixed-position** full-width Save button at the bottom of the viewport (not sticky-within-container — always visible while scrolling). Label is dynamic:
  - All qty = 0 → label `"Save"`, button **disabled**.
  - ≥1 qty > 0 → label `"Save N cards"` where N = sum of all entered quantities.
- [ ] Card-list scroll area has bottom padding equal to the Save button's height so the last row never sits beneath it.
- [ ] On Save: server upserts `inventory` rows for each entry where qty > 0 — find by `card_id` (or insert), set `quantity = existing_quantity + entered_qty` (additive). `override_price`, `calculated_price`, `last_exported_price` left untouched. Entries with qty = 0 are skipped silently.
- [ ] After successful save: toast `"Added N cards to {Set} ({Condition})."`, all qty inputs reset to 0, scope selectors stay set.
- [ ] Save button is disabled while a save is in flight (prevents double-submit).
- [ ] **Re-scoping mid-session** with pending entries (any qty > 0): auto-save the pending entries first, toast `"Saved before switching."`, then load the new scope. If auto-save fails: cancel the switch, render an `MfErrorBanner` at the top, leave the scope selector unchanged.
- [ ] Re-scoping with no pending entries (all qty = 0): silent switch, no save call.
- [ ] +/- button behavior: `+` increments by 1; `−` decrements by 1, floor 0; tapping the integer opens a numeric keypad for direct entry. Long-press does **not** auto-repeat.
- [ ] Pull-to-refresh disabled on the page (best-effort via `overscroll-behavior: contain` or platform-appropriate CSS).
- [ ] Mobile: tap targets ≥ 44 × 44 px; no horizontal scroll on a 375 px viewport; numeric keypad does not obscure the fixed Save button.
- [ ] Pest feature test `tests/Feature/Admin/AddCardsTest.php` covers:
