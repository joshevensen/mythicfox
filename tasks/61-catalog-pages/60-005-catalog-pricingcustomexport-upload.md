...
- [x] Clicking **Upload PricingCustomExport** opens a PrimeVue `Dialog` containing one `MfFileDropzone` accepting `.csv` (max 200MB).
- [x] Client-side validation: extension only (`.csv`). MIME validation happens server-side.
- [x] Submit posts a multipart request to a controller action (e.g. `POST /catalog/upload`) that:
...
- [x] On submit success: modal closes, toast displays *"PricingCustomExport queued — refreshing catalog…"*. Upload button enters in-flight state ("Importing…" with spinner badge) until the job reports completion.
- [x] On job completion: success toast *"Refreshed N cards across {Product Name}"*. Catalog table auto-reloads.
- [x] On server-side validation failure (wrong header, missing columns): `MfErrorBanner` at top of page with the parse error. The file is still saved to `files` for inspection (asserted in test).
- [x] Empty-state CTA on `/catalog` ("Your catalog is empty.") opens this same modal — share state via a `useCatalogUploadModal()` composable.
- [x] Mobile: upload button sticks to the bottom of the viewport on phones for one-thumb reach.
- [x] Pest feature test: authenticated POST to the upload endpoint with a fake CSV stores a `files` row with `type=import` and dispatches the import job (use `Bus::fake()` and `Storage::fake()`).
- [x] Pest feature test: posting a non-CSV file (e.g. `.txt`) returns 422.
- [x] Pest feature test: posting a malformed CSV still creates a `files` row but the response indicates failure (banner content asserted via flash data).
